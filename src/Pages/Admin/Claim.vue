...
// 차트
import MonthlyBarClaimchart from "@/components/MonthlyBarClaimchart.vue";
import MonthlycircleClaim from "@/components/MonthlycircleClaim.vue";
...
const statusCards = [
  {
    ...
    title: "전체 클레임",
    count: totalCount,
    desc: `이 달 총${totalCount}건의 클레임이 접수되었습니다.`,
...
`,
  },
  {
    status: "waiting", // ← 수정
    title: "상담 대기중",
    count: waitingCount, // 이 count도 실제 waitingCount로 바뀌어야 정확
    desc: `${waitingCount}건의 클레임이 현재 상담 배정 대기 중입니다.`,
...
`,
  },
  {
    ...
    title: "진행 중",
    count: assignedCount,
    desc: `${assignedCount}건의 클레임이 상담 잰행 중입니다.`,
...
`,
  },
  {
    ...
    title: "처리 완료",
    count: doneCount,
    desc: `${doneCount}건의 클레임이 상담을 통해 처리 완료되었습니다. `,
...
<template>
  <div class="adminreservation">
    ...
    <div class="table-wrap">
      ...
      <div class="searchbox websearchbox">
        ...
        <div class="searchbt profile-h4">
          <div class="statusbox">
            ...
            <label><input type="radio" value="confirmed" v-model="statusFilter" /> 재요청</label>
      ...
      <div class="tablelist">
        <h2 class="profile-h2">클레임 발생 현황</h2>
        <table class="table">
          <thead>
            <tr>
              <th>번호</th>
          ...
          <tbody>
            <tr v-for="item in paginatedList" :key="item.id">
              ...
              <td data-label="클레임아이디">{{ item.customer.claimId }}</td>
              <td data-label="사업개인">{{ item.store }}</td>
              <td data-label="카테고리">{{ item.customer.detailclaim }}</td>
              ...
              <td class="" data-label="청소일자">
                {{ item.date }}
              </td>
              <td class="relative" data-label="담당기사">
                <div class="flex items-center gap-2">
                  ...
                  <div class="flex gap-1">
                    <!-- 메일 버튼 -->
                    <button
                      @click="sendMail(item.worker)"
                      class="px-2 py-[2px] text-xs border border-stone-300 text-neutral-700 font-normal font-['Pretendard'] rounded hover:bg-neutral-100">
                      메일
                    </button>

                    <!-- SMS 버튼 -->
                    <button
                      @click="sendSMS(item.worker)"
                      class="px-2 py-[2px] text-xs border border-stone-300 text-neutral-700 font-normal font-['Pretendard'] rounded hover:bg-neutral-100">
                      SMS
                    </button>
                  </div>
